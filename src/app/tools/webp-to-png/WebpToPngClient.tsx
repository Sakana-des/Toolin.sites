      ...
      {files.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm md:text-base font-bold text-slate-800 dark:text-slate-200">
            Tarik file WEBP ke sini, atau klik untuk memilih
          </p>
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">
            Mendukung konversi banyak file sekaligus (bulk upload). Transparansi dipertahankan.
          </p>
        </div>
      ) : (
        /* Workspace list */
        <div className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={resetAll}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-855 hover:bg-slate-50 dark:hover:bg-slate-900 text-xs font-semibold text-slate-500 transition-colors"
              >
                <span>Hapus Semua</span>
              </button>
              <button
                onClick={() => fileInputRef.current?.click()}
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-650 hover:bg-blue-700 text-white text-xs font-bold transition-all"
              >
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">
                      WEBP: {formatSize(item.file.size)} 
                      {item.convertedSize && ` → PNG: ${formatSize(item.convertedSize)}`}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {item.status === "converting" && (
                    <span className="flex items-center gap-1 text-[10px] font-bold text-blue-650 dark:text-blue-400 animate-pulse uppercase">
                  ...
                  {item.status === "completed" ? (
                    ...
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 dark:border-slate-805 hover:bg-red-50 dark:hover:bg-red-955/30 hover:border-red-200 dark:hover:border-red-900/50 hover:text-red-650 transition-colors"
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-slate-200 dark:border-slate-800">
            {allCompleted ? (
              <button
                onClick={downloadAll}
                className="w-full sm:flex-1 flex items-center justify-center gap-2 rounded-xl bg-green-650 hover:bg-green-700 text-white font-bold py-3 px-4 shadow-lg shadow-green-500/20 transition-all hover:scale-[1.01]"
              >
                <Download className="h-5 w-5" />
                <span>Unduh Semua File PNG</span>
              </button>
            ) : (
              <button
                onClick={convertAll}
                className="w-full sm:flex-1 flex items-center justify-center gap-2 rounded-xl bg-blue-605 hover:bg-blue-700 text-white font-bold py-3 px-4 shadow-lg shadow-blue-500/20 transition-all hover:scale-[1.01]"
              >
                <RefreshCw className="h-5 w-5" />
                <span>Konversi ke PNG</span>
              </button>
            )}
